...
import org.springsource.restbucks.statemachine.StateMachineResultPostProcessor.CamundaStateMachine;
...
public class Restbucks {
	...
	public @Bean StateMachineResultPostProcessor processor(PersistentEntities entities, ProcessEngine engine) {
		...
		configuration.forAggregate(Order.class) //
				.withMethod("markPaid").boundTo("Message_PAYMENT_pay").withMethod("markInPreparation")
				.boundTo("Message_ORDER_START_PREPARATION") //
				.withMethod("markPrepared").boundTo("Message_ORDER_PREPARED") //
				.withMethod("markTaken").boundTo("Message_ORDER_TAKEN");

		return new StateMachineResultPostProcessor(new CamundaStateMachine(engine, entities), configuration);
	}
